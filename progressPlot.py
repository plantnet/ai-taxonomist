...
def process(log):
    ...
    for line in log:
        if line.startswith("Epoch:"):
            words = line.split("\t")
            match = re.match(
                r"Epoch: \[ *(?P<epoch>\d+)\]\[ *(?P<step>\d+)/(?P<max_step>\d+)\]",
                words[0],
            )
            ...
            try:
                words = line.split("\t")
                match = re.match(
                    r"Test: \[ *(?P<step>\d+)/(?P<max_step>\d+)\]", words[0]
                )
                step = int(match.group("step"))
            ...
            try:
                ...
                for i in range(2, len(words), 2):
                    name = words[i]
                    val = float(words[i + 1])
                    values["Accuracy"].setdefault(name, {}).setdefault("x", []).append(
                        epoch + 1
                    )
                    values["Accuracy"].setdefault(name, {}).setdefault("y", []).append(
                        val
                    )
            except:
                pass
...
def draw(val, args):
    ...
    ax1.tick_params(
        axis="y", labelcolor=colors[colorIdx], color=colors[colorIdx], direction="in"
    )
    if args.max_loss > 0:
...
def main():
    ...
    parser.add_argument(
        "--max-loss", type=float, default=0, help="clamp losses", required=False
    )
    parser.add_argument(
        "--train", action="store_true", help="Print train stats", required=False
    )
