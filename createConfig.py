"""
Create Snoop configuration files for a trained ai-taxonomist dataset
Use --help!
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Create Snoop configuration files for a trained ai-taxonomist dataset"
    )
    parser.add_argument("-d", "--data", help="output directory", required=True)
    offline = parser.add_argument_group(
        "offline", "parameters who impact snoop preparation"
    )
    ...
    offline.add_argument(
        "--compression", type=int, default=None, help="descriptors compressed size"
    )
    online = parser.add_argument_group(
        "online", "parameters who impact snoop at run time"
    )
    online.add_argument(
        "--gpu",
        type=int,
        default=[],
        nargs="*",
        help="Which gpu to use at run time, if any",
    )
    online.add_argument(
        "--port", type=int, default=8080, help="which port to launch the snoop on"
    )
    online.add_argument(
        "--snoop-threads",
        type=int,
        default=10,
        help="how many parallel request to handle",
    )
    online.add_argument(
        "--proxy",
        type=str,
        default="",
        help="proxy to use while downloading the images",
    )
